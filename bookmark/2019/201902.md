...
> settimeout の挙動など、ブラウザのタイマーはそれほど正確ではないですよ。
...
> Nuxt の基本的な機能を使いつつ、阿部寛に負けない performanse を達成するためのプロセス。blog サイトによく使う modules 群の解説があったり、コードは全て github で公開されてる。
...
> Reset CSS の考察。ブラウザデフォルト CSS とかまとまってたり、新しい animation をオフにする メディアクエリ prefers-reduced-motion とかも同梱されていたり。
...
> purge 150ms / config 5s ~ 10s で反映されるのでめっちゃ早い、fastly api integration 経由でイベント駆動で管理することが多いみたい。nikkei では、 それぞれのマイクロサービスのキャッシュを fastly に集約して、origin が死んでもキャッシュを返す、サービス同士が直接やりとりしないので障害があっても伝搬しない。cookie を fastly で デコードバリデートすることで 同一 URL で異なるキャッシュをもつ。
